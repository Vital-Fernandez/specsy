...
'''
Missing Z014v40_VMS_tracks_eleventst for the False rotation
Missing isotst_50Myr for plot_isochrones = True
Missing Z014_V00_sampling from plot_SED_time
output_age = 2.0 on pysb_inputs
...
# get the start time
st = time.time()
source_folder  = '/home/vital/Astrodata/pystarburst99/pySB99_source'
output_folder = '/home/vital/Astrodata/pystarburst99/compiled_spectra_v4'
...
if Z == 'MWC':
    file_path = f'{source_folder}/Z020_pySB99_files/'
    mass_grid = [300, 200., 150., 120., 85., 60., 40., 32., 25., 20., 15., 12., 9., 7., 5., 4., 3., 2.5, 2., 1.7, 1.5,
                 1.35, 1.25, 1.1, 1., 0.9, 0.8]
    ...
    if spectra_library == 'PoWR':
        ...
        if mass_loss_rate_option == 'DEFAULT':
            ...
            hires_radii = (((3.828 * 10 ** (26) * 10 ** hires_logls) / (
                        4 * 3.14 * 5.67 * 10 ** -8 * hires_teffs ** 4)) ** 0.5) / (7 * 10 ** 8)
...
if Z == 'MW':
    file_path = f'{source_folder}/Z014_pySB99_files/'
    if rot == True:
        mass_grid = [300., 250., 180., 120., 85., 60., 40., 32., 25., 20., 15., 12., 9., 7., 5., 4., 3., 2.5, 2., 1.7,
                     1.5, 1.35, 1.25, 1.1, 1., 0.9, 0.8]
        if IMF_mass_limits[-1] > 300.:
        ...
        mass_grid = [500., 300, 200., 150., 120., 85., 60., 50., 40., 32., 25., 23., 22., 20., 17., 15., 14., 12.,
                     11.75, 10., 9., 8., 7., 5., 4., 3., 2.5, 2., 1.7, 1.5, 1.35, 1.25, 1.1, 1., 0.9, 0.8]
...
if Z == 'LMC':
    file_path = f'{source_folder}/Z006_pySB99_files/'
    mass_grid = [300, 250., 180., 120., 85., 60., 40., 32., 25., 20., 15., 12., 9., 7., 5., 4., 3., 2.5, 2., 1.7, 1.5,
                 1.35, 1.25, 1.1, 1., 0.9, 0.8]
    if IMF_mass_limits[-1] > 300.:
...
if Z == 'SMC':
    file_path = f'{source_folder}/Z002_pySB99_files/'
    mass_grid = [120., 85., 60., 40., 32., 25., 20., 15., 12., 9., 7., 5., 4., 3., 2.5, 2., 1.7, 1.5, 1.35, 1.25, 1.1,
                 1., 0.9, 0.8]
...
if Z == 'IZw18':
    file_path = f'{source_folder}/Z0004_pySB99_files/'
    ...
    if spectra_library == 'PoWR':
        ...
        if mass_loss_rate_option == 'DEFAULT':
            ...
            hires_spec_reformed = np.genfromtxt(
                file_path + 'subsmc_ob_vd3_hires_uv_calib.txt')  # each spectrum is a row
            hires_radii = (((3.828 * 10 ** (26) * 10 ** hires_logls) / (
                        4 * 3.14 * 5.67 * 10 ** -8 * hires_teffs ** 4)) ** 0.5) / (7 * 10 ** 8)
...
if Z == 'XMP':
    file_path = f'{source_folder}/Z00001_pySB99_files/'
...
if Z == 'Z0':
    file_path = f'{source_folder}/Z00_pySB99_files/'
    if rot == True:
...
if save_output == True:
    os.makedirs(f'{output_folder}/{SBmodel_name}', exist_ok=True)
...
# Inputs
times_spectra_start = 0.1e6   # yrs — first nonzero age (log grid can't start at 0)
times_spectra_end   = times_spectra_end
n_spectra           = 50      # number of log-spaced output ages
refine              = 10      # fine interpolation steps per output interval

# Fine interpolation grid (log-spaced)
times_steps_log = np.geomspace(times_spectra_start, times_spectra_end,
                               num=(n_spectra - 1) * refine + 1)

# Output grid = exact subset of the fine grid (every `refine`-th point)
times_spectra_log = times_steps_log[::refine]

# Prepend the zero-age bootstrap step to both
times_steps   = np.concatenate(([0.0], times_steps_log))
times_spectra = np.concatenate(([0.0], times_spectra_log))
...
def calc_Nostars(IMF_masses, IMF_exponents, IMF_mass_limits):
    ...
    if N_IMF_intervals > 1:
        ...
        for exponent_index in range(len(IMF_exponents)):
            ...
            A_i = A_ic[exponent_index - 1] * (IMF_mass_limits[exponent_index] ** (
                        IMF_exponents[exponent_index] - IMF_exponents[exponent_index - 1]))
            # print(Aici)
        ...
        for exponent_index in range(len(IMF_exponents)):
            if IMF_exponents[exponent_index] == 2.0:
                ...
                k_i = (IMF_mass_limits[exponent_index + 1] ** (2.0 - IMF_exponents[exponent_index]) - IMF_mass_limits[
                    exponent_index] ** (2.0 - IMF_exponents[exponent_index])) / (2 - IMF_exponents[exponent_index])
            k_ic.append(k_i)
        ...
        A_ic = 1
        k_ic = (IMF_mass_limits[1] ** (2.0 - IMF_exponents[0]) - IMF_mass_limits[0] ** (2.0 - IMF_exponents[0])) / (
                    2 - IMF_exponents[0])
        Ak = A_ic * k_ic
    ...
    for mass_index in range(
            len(IMF_masses)):  # set width of bin for each individual IMF mass through upper and lower limits
    ...
    if N_IMF_intervals == 2:
        for mass_index in range(len(IMF_masses)):
            dens_i = A[1] * (
                        xmhigh[mass_index] ** (1 - IMF_exponents[1]) - xmlow[mass_index] ** (1 - IMF_exponents[1])) / (
                                 1 - IMF_exponents[1])
            dens.append(dens_i)
    else:
        for mass_index in range(len(IMF_masses)):
            dens_i = A * (
                        xmhigh[mass_index] ** (1 - IMF_exponents[0]) - xmlow[mass_index] ** (1 - IMF_exponents[0])) / (
                                 1 - IMF_exponents[0])
            dens.append(dens_i)
...
for i in range(len(track_temps)):
    ...
    track_H_abundances[i] = np.append(track_H_abundances[i],
                                      np.array([track_H_abundances[i][-1], track_H_abundances[i][-1]]))
    track_He_abundances[i] = np.append(track_He_abundances[i],
                                       np.array([track_He_abundances[i][-1], track_He_abundances[i][-1]]))
    track_12C_abundances[i] = np.append(track_12C_abundances[i],
                                        np.array([track_12C_abundances[i][-1], track_12C_abundances[i][-1]]))
    track_14N_abundances[i] = np.append(track_14N_abundances[i],
                                        np.array([track_14N_abundances[i][-1], track_14N_abundances[i][-1]]))
    track_16O_abundances[i] = np.append(track_16O_abundances[i],
                                        np.array([track_16O_abundances[i][-1], track_16O_abundances[i][-1]]))
    track_core_temps[i] = np.append(track_core_temps[i], np.array([track_core_temps[i][-1], track_core_temps[i][-1]]))
    track_mass_loss_rates[i] = np.append(track_mass_loss_rates[i],
                                         np.array([track_mass_loss_rates[i][-1], track_mass_loss_rates[i][-1]]))
...
reformed_spec_grid, spec_params_reform, spec_params_teff, spec_params_logl, spec_params_logg = reform_spec_grid(spectra,
                                                                                                                spec_params)
...
WC_reformed_spec_grid_powr, WC_spec_params_reform_powr, WC_spec_params_teff_powr, WC_spec_params_radius_powr, WC_spec_params_length_powr = reform_spec_grid_powr(
    WC_spectra_powr, WC_spec_params_powr)
WN_reformed_spec_grid_powr, WN_spec_params_reform_powr, WN_spec_params_teff_powr, WN_spec_params_radius_powr, WN_spec_params_length_powr = reform_spec_grid_powr(
    WN_spectra_powr, WN_spec_params_powr)
...
def integrate_spec_grid(reformed_spec_grid):
    integrated_spectra = []
    for i in range(len(reformed_spec_grid)):
        integrated_spectrum = np.trapezoid(reformed_spec_grid[i][:, 1], reformed_spec_grid[i][:, 0])
...
def integrate_spec_grid_powr(wave_grid, reformed_spec_grid):
    integrated_spectra = []
    for i in range(len(reformed_spec_grid)):
        integrated_spectrum = np.trapezoid(wave_grid, reformed_spec_grid[i, :])
        integrated_spectra.append(integrated_spectrum)
    return (integrated_spectra)
...
def interpolate_param(tracks_parameter, track_masses, run_speed_mode):
    ...
    for i in range(len(track_masses) - 1):
        ...
        for track_param, track_mass, grid_mass in zip(track_param_adjinterp, track_masses_adjinterp,
                                                      grid_masses_adjinterp):
...
grid_masses_adjinterp_total, grid_mass_loss_rates_adjinterp_total = interpolate_param(track_mass_loss_rates,
                                                                                      track_masses, run_speed_mode)
...
def get_timestep_0_ind(timestep):
    ...
    timestep_masses_unique = np.unique(timestep_masses,
                                       return_index=True)  # Gives unique initial masses to avoid duplication and adds index as second column

    timestep_masses_IMF_ind = [i for i in range(len(timestep_masses_unique[0])) if
                               timestep_masses_unique[0][i] < IMF_mass_limits[-1]]
...
def get_timestep_params(timestep, timestep_mass_ind):
    ...
    timestep_coher = ((timestep_12C_abundances_final / 12.) + (timestep_16O_abundances_final / 16.)) / (
                timestep_He_abundances_final / 4.)
    ...
    for i in range(len(timestep_masses_final)):
        if timestep_H_abundances_final[i] < 0.1:  # and timestep_temps_final[i] > 4.4:
            ...
            corrected_teff = timestep_core_teff_final + (WR_correction_factor - 1.0) * (
                        timestep_core_teff_final - timestep_teffs_calc[i])
    ...
    return (timestep_ages_final, timestep_temps_final, timestep_lums_final, timestep_masses_final,
            ...
            timestep_masses, timestep_cnr, timestep_coher, timestep_uncorr_teff_final)
...
def get_specsyn_params(timestep_temps_final, timestep_masses_final, timestep_lums_final):
    ...
    for i in range(len(timestep_temps_final)):
        ...
        logg_specsyn = np.log10(timestep_masses_final[i]) + (4. * timestep_temps_final[i]) - timestep_lums_final[
            i] - 10.6
...
def assign_spectra_to_grid_WR(timestep_temps_final, spec_params_reform, spec_params_teff, spec_params_logl,
                              timestep_lums_final, timestep_H_abundances_final, spec_params_logg, timestep_masses_final,
                              initial_masses, specsyn_cotests, specsyn_loggs, timestep_cnr, timestep_coher):
    ...
    for j in range(len(timestep_temps_final)):
        ...
        if timestep_lums_final[j] == -20.0:
        ...
        elif timestep_temps_final[j] > 4.4 and timestep_H_abundances_final[j] < 0.4 and initial_masses[
            j] > minimum_wr_mass:  # e.g. 20 for solar but can change
            ...
            for i in range(len(spec_params_reform)):
                ...
                distance_to_spec.append(abs(spec_params_teff[i] - 10 ** timestep_temps_final[j]) + abs(
                    spec_params_logg[i] - timestep_loggs_final[j]))
            nearest_spec_ind = np.argmin(distance_to_spec)
            ...
            for i in range(len(lowmass_params)):
                distance_to_spec.append((lowmass_teffs[i] - 10 ** timestep_temps_final[j]) ** 2 + (
                            lowmass_loggs[i] - timestep_loggs_final[j]) ** 2)
            nearest_spec_ind = np.argmin(distance_to_spec)
    ...
    return (assigned_integrated_spectra, assigned_spec_teff, assigned_spectra, population_choice, assigned_spec_logl,
            assigned_spec_logg)


def assign_spectra_to_grid_hires(timestep_temps_final, hires_teffs, hires_loggs, timestep_lums_final,
                                 timestep_H_abundances_final, timestep_masses_final, initial_masses, specsyn_cotests,
                                 specsyn_loggs):
    assigned_integrated_spectra = []
    assigned_spec_teff = []
    assigned_spec_logg = []
    ...
    for j in range(len(timestep_temps_final)):
        ...
        if timestep_lums_final[j] == -20.0:
        ...
        elif timestep_temps_final[j] > 4.4 and timestep_H_abundances_final[j] < 0.40 and initial_masses[
            j] >= minimum_wr_mass:  # e.g. 20 for solar but can change
            if timestep_H_abundances_final[j] > 0.098:
                ...
                assigned_flux_scaled = assigned_spectrum[:, 1] * (
                            ((3.0856e9) ** 2.0) / ((assinged_radius * 6.96e10) ** 2.0)) * 1.0e20
        ...
        # elif initial_masses[j] < 5. or timestep_teffs_final[j] < 2000:
            ...
            if spectra_library == 'FW':  # and timestep_teffs_final[j] > 17600:
                ...
                for i in range(len(hires_teffs)):
                    ...
                    distance_to_spec.append((timestep_temps_final[j] - np.log10(hires_teffs[i])) ** 2 + 5 * (
                                hires_loggs[i] - specsyn_loggs[j]) ** 2)
                nearest_spec_ind = np.argmin(distance_to_spec)
            ...
            if spectra_library == 'PoWR':
                ...
                for i in range(len(hires_teffs)):
                    # distance_to_spec.append(abs(hires_teffs[i] - 10**timestep_temps_final[j]) + abs(hires_loggs[i] - specsyn_loggs[j]))
                    # distance_to_spec.append(abs(hires_teffs[i] - 10**timestep_temps_final[j]) + abs(hires_loggs[i] - specsyn_loggs[j]))
                    distance_to_spec.append(
                        (timestep_temps_final[j] - hires_temps[i]) ** 2 + 5 * (hires_loggs[i] - specsyn_loggs[j]) ** 2)
                ...
                assigned_flux_scaled = assigned_spectrum[
                    :, 1]  # *(((3.0856e9)**2.0)/((assinged_radius*6.96e10)**2.0)) * 1.0e20
                assigned_flux_scaled = assigned_flux_scaled * timestep_teffs_final[j] / near_spec_temp
                assigned_spectra.append(np.column_stack((hires_wave_grid, assigned_flux_scaled)))
                assigned_cont.append(hires_cont[nearest_spec_ind])
                assigned_integrated_spectra.append(hires_int_spec[nearest_spec_ind])

    return (assigned_integrated_spectra, assigned_spec_teff, assigned_spec_logg, assigned_spectra, assigned_cont,
            hires_choice)
...
def specsyn(assigned_integrated_spectra, specsyn_bbfluxes, assigned_spectra, radii, No_stars):
    ...
    for i in range(len(test_assigned_flux)):
        assigned_flux_scaled_i = 12.566 * test_radii[i] * test_radii[i] / 1e20 * test_assigned_flux[i] * No_stars[
            i]  # * 3.14 * 4 * 5# /1e20
...
def specsyn_hires(assigned_integrated_spectra, specsyn_bbfluxes, assigned_spectra, assigned_cont, radii, No_stars,
                  population_nebular, assigned_hires_choice):
    assigned_flux = []
    assigned_cont_flux = []
    ...
    if spectra_library == 'PoWR':
        ...
        for i in range(len(assigned_flux)):
            assigned_flux_scaled_i = 12.566 * radii[i] ** 2 / 1e20 * assigned_flux[i] * No_stars[
                i]  # * 3.14 * 4 * 5# /1e20
            assigned_flux_scaled.append(assigned_flux_scaled_i)
            ...
            if assigned_hires_choice[i] == 'OB - PoWR':
                ...
                assigned_cont_scaled_i = 12.566 * radii[i] ** 2 / 1e20 * assigned_cont_flux[i] * No_stars[
                    i]  # * 3.14 * 4 * 5# /1e20
...
def calc_wind(timestep_temps, timestep_lums, timestep_masses, timestep_mdot, timestep_H, timestep_12C, timestep_14N,
              timestep_16O, inital_masses, No_stars, timestep_radii, timestep_cnr):
    ...
    timestep_coher = ((timestep_12C / 12.) + (timestep_16O / 16.)) / (
                timestep_H / 4.)  # WRONG NEEDS TO BE He INSTEAD OF H ABUNDANCE
    vinfs = []
    ...
    for i in range(len(timestep_temps)):
        ...
        #                vinfs.append(vinf)
            ...
            if gamma0 <= 0.:
                gamma0 = 1e-10
            vinf = 618. * np.sqrt(
                timestep_masses[i] / (10. ** (0.5 * timestep_lums[i] - 2. * timestep_temps[i] + 7.52)) * gamma0) * (
                               0.58 + 2.04 * (0.5 * timestep_lums[i] - 2. * timestep_temps[i] + 7.52))
            # timestep_mdot[i] = np.log10((10**timestep_mdot[i])/3)
            timestep_mdot[i] = np.log10((10 ** timestep_mdot[i]))
            mdot_leuveni = -5.52 + 2.39 * np.log10((10 ** timestep_lums[i]) / (10 ** 6)) - 1.48 * np.log10(
                timestep_masses[i] / 45.) + 2.12 * np.log10(timestep_teffs[i] / 45000.) + (
            ...
            if timestep_teffs[i] < 25883.:
                ...
                mdot_vink00i = -6.688 + 2.210 * np.log10(
                    ((10 ** timestep_lums[i]) / (1. * 10 ** 5))) - 1.339 * np.log10(
                    timestep_masses[i] / 30.) - 1.601 * np.log10(
                    (vinf_vink21i / timestep_vesc[i]) / 2.) + 1.07 * np.log10(
                    timestep_teffs[i] / 20000.)  # + 0.85*np.log10(1./1.)

            else:
                log_vinf_vink21i = 0.39 - 0.04 * timestep_lums[i] + 0.74 * timestep_temps[i]  # + 0.19*np.log10(1/1)
                vinf_vink21i = 10 ** log_vinf_vink21i
                mdot_vink00i = -6.697 + 2.194 * np.log10(
                    ((10 ** timestep_lums[i]) / (1. * 10 ** 5))) - 1.313 * np.log10(
                    timestep_masses[i] / 30.) - 1.226 * np.log10(
                    (vinf_vink21i / timestep_vesc[i]) / 2.) + 0.933 * np.log10(timestep_teffs[i] / 40000.) - 10.92 * (
                                   np.log10(timestep_teffs[i] / 40000.)) ** 2  # + 0.85*np.log10(1./1.)
    ...
    return (vinfs_Z, wind_power_calc, wpower, wind_mom_calc, wmom, wmom_vink, vinf_vink21, mdot_vink00, timestep_vesc,
            wmom_leuven, wmom_xshootu, wpower_xshootu)
...
def get_uv_slope(uv_wave, uv_flux):
    ...
    ind_uvslope_1 = [i for i in range(len(uv_wave)) if
                     uv_wave[i] >= uvslope_bounds_1[0] and uv_wave[i] <= uvslope_bounds_1[1]]
    ind_uvslope_2 = [i for i in range(len(uv_wave)) if
                     uv_wave[i] >= uvslope_bounds_2[0] and uv_wave[i] <= uvslope_bounds_2[1]]
    ind_uvslope_3 = [i for i in range(len(uv_wave)) if
                     uv_wave[i] >= uvslope_bounds_3[0] and uv_wave[i] <= uvslope_bounds_3[1]]
    ind_uvslope_4 = [i for i in range(len(uv_wave)) if
                     uv_wave[i] >= uvslope_bounds_4[0] and uv_wave[i] <= uvslope_bounds_4[1]]
    ind_uvslope_5 = [i for i in range(len(uv_wave)) if
                     uv_wave[i] >= uvslope_bounds_5[0] and uv_wave[i] <= uvslope_bounds_5[1]]
    ind_uvslope_6 = [i for i in range(len(uv_wave)) if
                     uv_wave[i] >= uvslope_bounds_6[0] and uv_wave[i] <= uvslope_bounds_6[1]]

    ind_uvslope_full = np.concatenate(
        (ind_uvslope_1, ind_uvslope_2, ind_uvslope_3, ind_uvslope_4, ind_uvslope_5, ind_uvslope_6))
...
def colours(population_flux):
    V_wave = [50., 4750., 4800., 4850., 4900., 4950., 5000., 5050., 5100., 5150., 5200., 5250., 5300., 5350., 5400.,
              5450., 5500., 5550., 5600., 5650., 5700., 5750., 5800., 5850., 5900., 5950., 6000., 6050., 6100., 6150.,
              6200., 6250., 6300., 6350., 6400., 6450., 6500., 6550., 6600., 6650., 6700., 6750., 6800., 6850., 6900.,
              6950., 7000., 7050., 7100., 7150., 7200., 7250., 7300., 7350., 7400., 2000000.]
    V_profile = [0.000, 0.000, 0.030, 0.084, 0.163, 0.301, 0.458, 0.630, 0.780, 0.895, 0.967, 0.997, 1.000, 0.988,
                 0.958, 0.919, 0.877, 0.819, 0.765, 0.711, 0.657, 0.602, 0.545, 0.488, 0.434, 0.386, 0.331, 0.289,
                 0.250, 0.214, 0.181, 0.151, 0.120, 0.093, 0.069, 0.051, 0.036, 0.027, 0.021, 0.018, 0.016, 0.014,
                 0.012, 0.011, 0.010, 0.009, 0.008, 0.007, 0.006, 0.005, 0.004, 0.003, 0.002, 0.001, 0.000, 0.000]

    U_wave = [50., 3050., 3100., 3150., 3200., 3250., 3300., 3350., 3400., 3450., 3500., 3550., 3600., 3650., 3700.,
              3750., 3800., 3850., 3900., 3950., 4000., 4050., 4100., 4150., 4200., 2000000.]
    U_profile = [0.000, 0.000, 0.020, 0.077, 0.135, 0.204, 0.282, 0.385, 0.493, 0.600, 0.705, 0.820, 0.900, 0.959,
                 0.993, 1.000, 0.975, 0.850, 0.645, 0.400, 0.223, 0.125, 0.057, 0.005, 0.000, 0.000]

    I_wave = [50., 7000., 7100., 7200., 7300., 7400., 7500., 7600., 7700., 7800., 7900., 8000., 8100., 8200., 8300.,
              8400., 8500., 8600., 8700., 8800., 8900., 9000., 9100., 9200., 2000000.]
    I_profile = [0.00000, 0.00000, 0.02400, 0.23200, 0.55500, 0.78500, 0.91000, 0.96500, 0.98500, 0.99000, 0.99500,
                 1.00000, 1.00000, 0.99000, 0.98000, 0.95000, 0.91000, 0.86000, 0.75000, 0.56000, 0.33000, 0.15000,
                 0.03000, 0.00000, 0.00000]

    B_wave = [50., 3600., 3650., 3700., 3750., 3800., 3850., 3900., 3950., 4000., 4050., 4100., 4150., 4200., 4250.,
              4300., 4350., 4400., 4450., 4500., 4550., 4600., 4650., 4700., 4750., 4800., 4850., 4900., 4950., 5000.,
              5050., 5100., 5150., 5200., 5250., 5300., 5350., 5400., 5450., 5500., 5550., 2000000.]
    B_profile = [0.000, 0.000, 0.006, 0.030, 0.060, 0.134, 0.302, 0.567, 0.841, 0.959, 0.983, 0.996, 1.000, 0.996,
                 0.987, 0.974, 0.957, 0.931, 0.897, 0.849, 0.800, 0.748, 0.698, 0.648, 0.597, 0.545, 0.497, 0.447,
                 0.397, 0.345, 0.297, 0.252, 0.207, 0.166, 0.129, 0.095, 0.069, 0.043, 0.024, 0.009, 0.000, 0.000]
    ...
    V_value = np.trapezoid(wave_grid, V_data)
    U_value = np.trapezoid(wave_grid, U_data)
    I_value = np.trapezoid(wave_grid, I_data)
    B_value = np.trapezoid(wave_grid, B_data)
...
def get_SN_rate(population_SN_ind):
    ...
    for i in range(
            len(population_SN_ind)):  # population_SN_ind gives the list of all indices for masses that have gone SN so far
        if i == len(population_SN_ind) - 1:
            population_new_SN_mass_inds.append([])
            break
        pop_SN_ind_test = population_SN_ind[i + 1][:-len(population_SN_ind[
                                                             i])]  # this just finds all the mass indices that have gone supernova in the latest timestep
...
for i in range(len(times_steps)):
    ...
    timestep_ages_final, timestep_temps_final, timestep_lums_final, timestep_masses_final, timestep_H_abundances_final, timestep_loggs_final, timestep_mass_loss_rates_final, timestep_12C_abundances_final, timestep_14N_abundances_final, timestep_16O_abundances_final, timestep_mass_test, timestep_cnr, timestep_coher, timestep_uncorr_teffs = get_timestep_params(
        timestep_ex, timestep_mass_ind)

    if i == 0:
        if input_imf_option == True:
            initial_masses = timestep_masses_final
            No_stars, c_masses, dens, xmhigh, xmlow = calc_Nostars(timestep_masses_final, IMF_exponents,
                                                                   IMF_mass_limits)  # second print
    ...
    specsyn_teffs, specsyn_loggs, specsyn_radii, specsyn_cotests, specsyn_bbfluxes = get_specsyn_params(
        timestep_temps_final, timestep_masses_final, timestep_lums_final)
    assigned_integrated_spectra, assigned_spec_teff, assigned_spectra, population_choice, assigned_spec_logl, assigned_spec_logg = assign_spectra_to_grid_WR(
        timestep_temps_final, spec_params_reform, spec_params_teff, spec_params_logl, timestep_lums_final,
        timestep_H_abundances_final, spec_params_logg, timestep_masses_final, initial_masses, specsyn_cotests,
    ...
    #    population_flux = specsyn(assigned_integrated_spectra, specsyn_bbfluxes, assigned_spectra, timestep_radii_final)
    population_flux, assigned_flux_scaled = specsyn(assigned_integrated_spectra, specsyn_bbfluxes, assigned_spectra,
                                                    specsyn_radii, No_stars)
    timestep_vinfs, timestep_windpowers_calc, timestep_windpowers, timestep_windmoms_calc, timestep_windmoms, timestep_windmoms_vink, timestep_vinfs_vink, timestep_mdots_vink, timestep_vescs, timestep_windmoms_leuven, timestep_windmoms_xshootu, timestep_windpowers_xshootu = calc_wind(
        timestep_temps_final, timestep_lums_final, timestep_masses_final, timestep_mass_loss_rates_final,
        timestep_H_abundances_final, timestep_12C_abundances_final, timestep_14N_abundances_final,
        timestep_16O_abundances_final, initial_masses, No_stars, timestep_radii_final, timestep_cnr)
    ...
    timestep_Ha_ew, timestep_Hb_ew, timestep_Pb_ew, timestep_Bg_ew, timestep_Ha_luminosity, timestep_Ha_continuum_flux = get_ew(
        reformed_spec_grid[0][:, 0], population_flux_total, population_ion_flux[1][0])
    ...
    timestep_uv_slope_x, timestep_uv_slope_y, timestep_uv_slope_beta = get_uv_slope(
        np.log10(reformed_spec_grid[0][:, 0]), np.log10(population_flux_total) + 20.)
    ...
    if spectra_library == 'WM':
        hires_assigned_integrated_spectra, hires_assigned_spec_teff, hires_assigned_spec_logg, hires_assigned_spectra, hires_assigned_cont, assigned_hires_choice = assign_spectra_to_grid_hires(
            timestep_temps_final, hires_teffs, hires_loggs, timestep_lums_final, timestep_H_abundances_final,
            timestep_masses_final, initial_masses, specsyn_cotests, specsyn_loggs)
        hires_population_flux, hires_population_flux_wneb, hires_population_flux_norm, hires_population_nebular = specsyn_hires(
            assigned_integrated_spectra, specsyn_bbfluxes, hires_assigned_spectra, hires_assigned_cont, specsyn_radii,
            No_stars, population_continuum, assigned_hires_choice)
        ...
        hires_assigned_integrated_spectra, hires_assigned_spec_teff, hires_assigned_spec_logg, hires_assigned_spectra, hires_assigned_cont, assigned_hires_choice = assign_spectra_to_grid_hires(
            timestep_temps_final, hires_teffs, hires_loggs, timestep_lums_final, timestep_H_abundances_final,
            timestep_masses_final, initial_masses, specsyn_cotests, specsyn_loggs)
        hires_population_flux, hires_population_flux_norm, hires_population_nebular = specsyn_hires(
            assigned_integrated_spectra, specsyn_bbfluxes, hires_assigned_spectra, hires_assigned_cont, specsyn_radii,
            No_stars, population_continuum, assigned_hires_choice)
        population_hires_flux_iterations.append(hires_population_flux)
        population_hires_flux_iterations_send.append(np.log10(hires_population_flux + 1.0e-35) + 20.)
    ...
    if plot_SN_rate == True:
        ind_SN_masses = [i for i in range(len(timestep_lums_final)) if
                         timestep_lums_final[i] == -20.0 and initial_masses[i] <= maximum_SN_mass]
...
if save_output == True:
    with open(f'{output_folder}/{SBmodel_name}' + '/input.txt', 'w') as inputs_file:
    ...
    np.savetxt(f'{output_folder}/{SBmodel_name}' + '/timesteps.txt', times_spectra[1:])
...
if plot_isochrones == True:
    ...
    sc = ax.scatter(population_uncorr_teffs[40] / 1000, population_lums[40], s=1, c=population_H_abundances[200],
                    cmap='plasma_r', vmin=Hmin, vmax=Hmax)
    ...
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/plot_isochrones.png')


    def plot_grid_tracks(track_xparam, track_yparam, grid_xparam_adjinterp_total, grid_yparam_adjinterp_total, title,
                         xlabel, ylabel, length, flipx):
        '''
        ...
        plt.legend()
        # plt.show()
        plt.savefig(f'{output_folder}/{SBmodel_name}/plot_grid_tracks.png')
        return


    plot_grid_tracks(track_temps[5:10], track_lums[5:10], grid_temps_adjinterp_total, grid_lums_adjinterp_total,
                     'Temperature and Luminosity tracks', 'Teff [K]', 'Luminosity [Lsol]', 22, 'yes')
...
if plot_SED_with_time == True:
    ...
    def read_pySB99_model(model_dir):
        ...
        return (ion_flux_HI, ion_flux_HeI, ion_flux_HeII, Lbol, uvslope, Ha_ew, windmom, windpower, SED, SED_wavelength,
                SED_times, SED_neb, spectrum, spectrum_norm, spectrum_wavelength)


    pySB_model_name = 'pysb_testmodJul29'

    ion_flux_HI, MWC_ion_flux_HeI, MWC_ion_flux_HeII, MWC_Lbol, MWC_uvslope, MWC_Ha, MWC_windmom, MWC_windpower, SED, SED_wavelength, SED_times, SED_neb, spectrum, spectrum_norm, spectrum_wavelength = read_pySB99_model(
        pySB_model_name)
    ...
    plt.style.use('default')
    ax = fig.add_subplot(111)
    ax.plot(SED_wavelength, 10 ** SED_neb[spec_time * 10, :],
            label='MW SED - ' + str(SED_times[100] / 10 ** 6) + 'Myrs', color='#674ea7')
    ...
    # plt.ylim(34,40)
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/plot_SED_with_time.png')
    ...
    if save_output == True:
        ...
        np.save(f'{output_folder}/{SBmodel_name}' + '/pySB_SED_stellar.npy', np.array(population_flux_iterations_send_save))
        np.save(f'{output_folder}/{SBmodel_name}' + '/pySB_SED_stellar_and_nebular.npy', population_flux_total_iterations_send_save)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/pySB_SED_stellar' + str(int(spec_time)) + '_Myr.txt',
                   np.column_stack((spectrum_wave, SED_plot_choice_flux)))
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/pySB_SED_stellar_and_nebular' + str(int(spec_time)) + '_Myr.txt',
                   np.column_stack((spectrum_wave, SED_plot_total_choice_flux)))
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/SED_wavelength.txt', spectrum_wave)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/SED_times.txt', times_spectra)
...
if plot_hires_spectra == True:
    ...
    #     SBhiresind_timestep = [i for i in range(len(SBhirestime)) if
    #                            SBhirestime[i] < SBhirestime_val + 10 and SBhirestime[i] > SBhirestime_val - 10]
    ...
    # ax.plot(hires_wave_grid, 10 ** spectrum[hires_spec_time * 10, :], label='pySB t=' + str(hires_spec_time) + 'Myr',
    #         color='tab:green')
    #
    ...
    # plt.style.use('default')
    # ax = fig.add_subplot(111)
    # ax.plot(hires_wave_grid, hires_plot_choice_flux_norm, label='pySB t=' + str(hires_spec_time) + 'Myr',
    #         color='tab:red')
    # ax.set_title('Hires ifa spectra', fontsize=12)
    # plt.xlim(900, 2000)
    # plt.xlabel('Wave', fontsize=12)
    ...
    if save_output == True:
        ...
        hires_flux_wneb_iterations_send_save = np.array(population_hires_flux_wneb_iterations_send)[
            ind_hires_spectra_output]
        hires_flux_norm_iterations_send_save = np.array(population_hires_flux_norm_iterations)[ind_hires_spectra_output]

        np.save(f'{output_folder}/{SBmodel_name}' + '/pySB_hires_spectrum.npy', hires_flux_wneb_iterations_send_save)
        np.save(f'{output_folder}/{SBmodel_name}' + '/pySB_hires_spectrum_without_neb.npy', hires_flux_iterations_send_save)
        np.save(f'{output_folder}/{SBmodel_name}' + '/pySB_hires_norm_spectrum.npy', hires_flux_norm_iterations_send_save)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/pySB_spectrum_' + str(int(hires_spec_time)) + '_Myr.txt',
                   np.column_stack((hires_wave_grid, hires_plot_choice_flux)))
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/pySB_norm_spectrum_' + str(int(hires_spec_time)) + '_Myr.txt',
                   np.column_stack((hires_wave_grid, hires_plot_choice_flux_norm)))
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/spectrum_wavelength.txt', hires_wave_grid)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/times_spectra.txt', times_spectra)
...
if plot_ion_flux == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/Q_HI.png')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/Q_HeI.png')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/HeII_Q.png')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/L_over_time.png')

    if save_output == True:
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/bololum.txt', population_ion_L_flux_iterations)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/ion_flux_HI.txt', population_ion_HI_flux_iterations)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/ion_flux_HeI.txt', population_ion_HEI_flux_iterations)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/ion_flux_HeII.txt', population_ion_HEII_flux_iterations)
...
if plot_wind == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/plot_wind.png')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/plot_wind_over_time.png')

    if save_output == True:
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/windpower.txt', population_windpowers)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/windmom.txt', population_windmoms)
...
if plot_uv_slope == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/plot_uv_slope.png')

    if save_output == True:
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/uv_slope.txt', population_uv_slopes_beta)
...
if plot_ew == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/plot_ew.png')

    if save_output == True:
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/Haew.txt', population_Ha_ew)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/Hbew.txt', population_Hb_ew)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/Pbew.txt', population_Pb_ew)
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/Bgew.txt', population_Bg_ew)
...
if plot_colours == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/plot_colours.png')

    colours_output = np.column_stack(
        (population_Vmag, population_Umag, population_Imag, population_Bmag, population_absVmag))
    colours_header = 'V, U, I, B, M_V'

    if save_output == True:
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/colours.txt', colours_output, header=colours_header)
...
if plot_SN_rate == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/plot_SN_rate.png')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f'{output_folder}/{SBmodel_name}/plot_SN_rate_over_time.png')

    if save_output == True:
        np.savetxt(f'{output_folder}/{SBmodel_name}' + '/SNrate.txt', np.column_stack((times_steps, SN_rates)))
