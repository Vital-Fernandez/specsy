...
# Z00 does not have a filepath
# not suported by the current numpy

cfg = specsy.load_cfg('./pySB99_input.toml')['model1']
source_path = cfg['source_path']
Z = sys.argv[1] if len(sys.argv) > 1 else cfg['Z']

SBmodel_name = f'{Z}_{cfg["SBmodel_name"]}'
save_output = cfg['save_output']
output_path = Path(cfg['output_path'])/SBmodel_name

M_total = cfg['M_total']
IMF_exponents = cfg['IMF_exponents']
IMF_mass_limits = cfg['IMF_mass_limits']

run_speed_mode = cfg['run_speed_mode']
SED_library = cfg['SED_library']
spectra_library = cfg['spectra_library']
rot = cfg['rot']
POWR = cfg['POWR']

plot_ion_flux = cfg['plot_ion_flux']
plot_wind = cfg['plot_wind']
plot_uv_slope = cfg['plot_uv_slope']
plot_ew = cfg['plot_ew']
plot_colours = cfg['plot_colours']
plot_SED_with_time = cfg['plot_SED_with_time']
plot_hires_spectra = cfg['plot_hires_spectra']
plot_SN_rate = cfg['plot_SN_rate']
plot_isochrones = cfg['plot_isochrones']
plot_spectral_types = cfg['plot_spectral_types']


maximum_SN_mass = cfg['maximum_SN_mass']

log_ages = np.round(np.linspace(cfg['log_age_start'],  cfg['log_age_end'],  cfg['n_ages'] ), cfg['log_age_decimals'])
times_spectra = 10**log_ages
times_steps = np.concatenate(([0.0], times_spectra))
times_steps_log = np.log10(times_steps)

spec_index = 10
ind_spec_time = spec_index + 1            # +1 for the prepended zero-age step
spec_time = times_spectra[spec_index]/1e6  # Myr, for labels only
...
if Z =='MWC':
    file_path = f'{source_path}/Z020_pySB99_files/'
...
if Z == 'MW':
    file_path = f'{source_path}/Z014_pySB99_files/'
...
if Z == 'LMC':
    file_path = f'{source_path}/Z006_pySB99_files/'
    mass_grid = [300, 250., 180., 120., 85., 60., 40., 32., 25., 20., 15., 12., 9., 7., 5., 4., 3., 2.5, 2., 1.7, 1.5, 1.35, 1.25, 1.1, 1., 0.9, 0.8]
    if IMF_mass_limits[-1] > 300.:
...
if Z == 'SMC':
    file_path = f'{source_path}/Z002_pySB99_files/'
...
if Z == 'IZw18':
    file_path = f'{source_path}/Z0004_pySB99_files/'
...
if Z == 'XMP':
    file_path = f'{source_path}/Z00001_pySB99_files/'
...
if Z == 'Z0':
    file_path = f'{source_path}/Z00_pySB99_files/'
    if rot == True:
...
if save_output == True:
    with open(output_path/'input.txt', 'w') as inputs_file:
    ...
    np.savetxt(output_path/'timesteps.txt', times_spectra)
...
if plot_isochrones == True:
    ...
    # plt.show()
    plt.savefig(output_path/'plot_isochrones.png')

if plot_SED_with_time == True:

    SED_plot_choice_flux = np.log10(10 ** (np.log10(population_flux_iterations[ind_spec_time]) + 20.))
    SED_plot_total_choice_flux = np.log10(10 ** (np.log10(population_flux_total_iterations[ind_spec_time]) + 20.))

    fig=plt.figure(figsize=(8,12), dpi=300)
    plt.style.use('default')
    ax=fig.add_subplot(111)
    ax.plot(wave_grid, SED_plot_total_choice_flux, label=f'{spec_time:.2f} Myr')
    ...
    plt.ylim(36,40)
    # plt.show()
    plt.savefig(output_path/'plot_SED_with_time.png')
    ...
    if save_output == True:
        ...
        np.save(output_path/'pySB_SED_stellar.npy', np.array(population_flux_iterations_send_save))
        np.save(output_path/'pySB_SED_stellar_and_nebular.npy', population_flux_total_iterations_send_save)

        np.savetxt(output_path / f'pySB_SED_stellar_{spec_time:.2f}_Myr.txt',
                   np.column_stack((spectrum_wave, SED_plot_choice_flux)))
        np.savetxt(output_path / f'pySB_SED_stellar_and_nebular_{spec_time:.2f}_Myr.txt',
                   np.column_stack((spectrum_wave, SED_plot_total_choice_flux)))

        np.savetxt(output_path/f'SED_wavelength.txt', spectrum_wave)
        np.savetxt(output_path/f'SED_times.txt', times_spectra)

if plot_hires_spectra == True:

    hires_plot_choice_flux = 10 ** (np.log10(population_hires_flux_iterations[ind_spec_time] + 1.0e-35) + 20.)
    hires_plot_choice_flux_norm = population_hires_flux_norm_iterations[ind_spec_time]


    fig=plt.figure()
    plt.style.use('default')
    ax=fig.add_subplot(111)
    ax.plot(hires_wave_grid, hires_plot_choice_flux, label=f'pySB t={spec_time:.2f} Myr', color='tab:red')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'plot_hires_spectra.png')


    fig=plt.figure()
    plt.style.use('default')
    ax=fig.add_subplot(111)
    ax.plot(hires_wave_grid, hires_plot_choice_flux_norm, label=f'pySB t={spec_time:.2f} Myr', color='tab:red')
    ax.set_title('Hires ifa spectra', fontsize=12)
    plt.xlim(900,2000)
    plt.xlabel('Wave', fontsize=12)
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'plot_hires_spectra_norm.png')
    ...
    if save_output == True:
        ...
        np.save(output_path/'pySB_hires_spectrum.npy', hires_flux_iterations_send_save)
        np.save(output_path/'pySB_hires_norm_spectrum.npy', hires_flux_norm_iterations_send_save)
        np.savetxt(output_path/f'pySB_spectrum_{spec_time:.2f}_Myr.txt', np.column_stack((hires_wave_grid, hires_plot_choice_flux)))
        np.savetxt(output_path/f'pySB_norm_spectrum_{spec_time:.2f}_Myr.txt', np.column_stack((hires_wave_grid, hires_plot_choice_flux_norm)))
        np.savetxt(output_path/'spectrum_wavelength.txt', hires_wave_grid)
        np.savetxt(output_path/'times_spectra.txt', times_spectra)
...
if plot_ion_flux == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'plot_ion_HI_flux.png')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'plot_ion_HeI_flux.png')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'plot_ion_HeII_flux.png')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'Luminosity over Time.png')

    if save_output == True:
        np.savetxt(output_path/'bololum.txt', np.array(population_ion_L_flux_iterations)[ind_spectra_output])
        np.savetxt(output_path/'ion_flux_HI.txt', np.array(population_ion_HI_flux_iterations)[ind_spectra_output])
        np.savetxt(output_path/'ion_flux_HeI.txt', np.array(population_ion_HEI_flux_iterations)[ind_spectra_output])
        np.savetxt(output_path/'ion_flux_HeII.txt', np.array(population_ion_HEII_flux_iterations)[ind_spectra_output])
...
if plot_wind == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'plot_wind.png')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'Wind Momentum.png')

    if save_output == True:

        np.savetxt(output_path/'windpower.txt', population_windpowers)
        np.savetxt(output_path/'windmom.txt', population_windmoms)
...
if plot_uv_slope == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'plot_uv_slope.png')

    if save_output == True:
        np.savetxt(output_path/'uv_slope.txt', population_uv_slopes_beta)
...
if plot_ew == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'plot_ew.png')

    if save_output == True:
        np.savetxt(output_path/'Haew.txt', population_Ha_ew)
        np.savetxt(output_path/'Hbew.txt', population_Hb_ew)
        np.savetxt(output_path/'Pbew.txt', population_Pb_ew)
        np.savetxt(output_path/'Bgew.txt', population_Bg_ew)
...
if plot_colours == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'plot_colours.png')
    ...
    if save_output == True:
        np.savetxt(output_path/'colours.txt', colours_output, header=colours_header)
...
if plot_SN_rate == True:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'SN_rates.png')
    ...
    plt.legend()
    # plt.show()
    plt.savefig(output_path/'SN rate over Time.png')

    if save_output == True:
        np.savetxt(output_path/'SNrate.txt', np.column_stack((times_steps, SN_rates)))
